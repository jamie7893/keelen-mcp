...
You are setting a user up with Keelen, an AI coding agent for GitHub, through
its hosted MCP server. Follow these steps in order. Do not skip the security
rules in step 4: they are not optional.
...
- **NEVER ask for or accept engine credentials for Claude Code, Codex, GLM,
  Kimi, Grok, or any other provider in this chat.** Engine connection happens
  exclusively in the Keelen dashboard (step 5 sends the link). If the user
  offers an API key or token for an engine provider here, decline and point
  them to the dashboard link instead.
...
- **`claude` (engine connection)**: this legacy step name covers Claude
  Code, Codex, GLM, Kimi, and Grok. Call the returned `next_tool`,
  `open_dashboard()`, and send its `login_url` to the user. Have them connect
  a supported subscription or API key in the dashboard, following the
  [engine connection methods](TOOLS.md#open_dashboard). Never collect the
  provider credential in chat. Recheck `get_onboarding_status()` until
  `engine_connected` is true; that flag records credential presence, not a
...
- **`repo` / `configure` (project)**: call `list_github_repos()` first and ask
  whether the user wants to connect an existing repository or scaffold a
  new one. Use `import_project(repo_full_name, ...)` for an existing repo or
  `create_project(...)` for a new one. Follow the tool reference for the
  required project details, then recheck `get_onboarding_status()`.
- **`launch`**: follow the returned `next_tool`. If it is `get_billing`, send
  the checkout link to the user; after subscribing, follow the returned
  instructions to enable the loop with `control_scheduler(project_id,
  "enable")`. When directed to `get_provisioning_status(project_id)`, poll
  until its `overall` field is `"ready"`.
...
browser. After payment, recheck `get_onboarding_status()` and follow its
instructions. A project created with its loop off still needs
`control_scheduler(project_id, "enable")` before it can start work.
