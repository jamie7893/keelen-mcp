...
from the repository's account, the repository moved). For live health read
[`project_status`](#project_status)'s `pm_lane.repo_access`.
...
While `next_action` is `wait`, two extra fields say *why* the intake lane is
idle and *when* it next acts, so a normal timed wait is distinguishable from a
stall:

| Field | Meaning |
|---|---|
| `intake_lane_reason` | `debounce` (waiting out the quiet period after your most recent submit), `in_flight` (a batch is running now), `daily_cap` (the per-project daily thread cap is reached), `backoff` (a timed hold after an upstream quota limit), `no_pending` (nothing queued), `spawn` (eligible — the next tick takes it) |
| `intake_next_eligible_at` | ISO-8601 UTC instant the wait ends, for the timed reasons (`debounce`, `backoff`, `daily_cap`). `null` for the others, whose end is not a knowable instant |

A batch of submissions restarts the `debounce` window on each one, so several
requests sent together begin processing a few minutes after the **last** of
them — that is normal, and `intake_next_eligible_at` is the moment to expect it.
...
This REPLACES the whole document; it does not append. Read the current text
with [`get_product_vision`](#get_product_vision) first, edit it, and set the
full result back.
...
Reads a project's current product vision. Returns `{project_id,
product_vision_md, updated_at}`; `product_vision_md` is `null` when no vision
has been set. Call this before `set_product_vision` — the setter replaces the
whole document, so writing without reading first discards whatever the user
already recorded.
...
This REPLACES the whole document; it does not append. Read the current text
with [`get_product_goal`](#get_product_goal) first, edit it, and set the full
result back.
...
Reads a project's current product goal. Returns `{project_id,
product_goal_md, updated_at}`; `product_goal_md` is `null` when no goal has
been set. Call this before `set_product_goal` — the setter replaces the whole
document, so writing without reading first discards whatever the user already
recorded.
...
**Params:** `project_id: str`, `task_id: str`, `reason: str`,
`superseded_by_task_id: str` (optional), `superseded_by_pr_number: int`
(optional)

Closes a task that should not be built — a duplicate, or work that already
shipped in another PR. The task is marked `cancelled` and keeps all of its
history (acceptance criteria, QA steps, iterations); nothing is deleted.
...
already-closed task changes nothing.

The response echoes `open_tasks` — how many tasks remain open on the project,
counted after the close commits. Compare it against what you expected: if it
did not drop, the ticket was already closed. This is the same count
`project_status` reports, and neither counts a `cancelled` task as open.

Prefer this over leaving a dead task on the board. Unfinished tasks count
against a project's planning capacity, so stale duplicates quietly stop new
roadmap items from being expanded.
...
Reports a project's open task count, iterations run today, `prs_merged_today`,
`ui_review`, `pass_rate`, `verify_decided_14d` / `verify_errored_14d`, open
...
scenario count and derived screenshot-matrix count against the project's
platform caps. Both caps are enforced at push time, not in your CI, so a
manifest sitting over cap fails every push while CI stays green. Absent when
the project is not `web_app` or the manifest cannot be read.

**`pass_rate`** is the LOCAL VERIFICATION pass rate over the last 14 days of
dev/QA iterations — the share whose verification step passed, out of those that
reached a verdict. It is NOT a delivery measure and is structurally independent
of merges: planning iterations carry no verification verdict at all, and a pull
request can merge with a failed one. Use `prs_merged_today` to answer "is this
project shipping".

**`verify_decided_14d`** and **`verify_errored_14d`** are `pass_rate`'s own
denominator and its sandbox-timeout share: the same 14-day window and the same
`passed`/`failed`/`errored` verdict set the ratio uses. `errored` is the local
verify sandbox running out of its time budget, not a code verdict — when it
dominates `verify_decided_14d`, `pass_rate` is measuring the verify budget, not
the code.

**`pm_lane`** is `{last_pm_iter_at, repo_access}` and answers "is the PLANNING
lane actually working". `repo_access` is `"ok"` or `"failing"`; `"failing"`
means the lane that turns roadmap items into tasks cannot read the repository
from GitHub, even when runs are still starting and `github_connected` reads
true. Freshness alone cannot show this: the planning lane's repository reads
fail quietly, so a planning run that saw nothing still finishes successfully and
still moves `last_pm_iter_at` forward. On `"failing"`, re-authorize the keelen
GitHub App on the repository's account, or reconnect GitHub in Settings.

Also carries `intake_lane_reason` and `intake_next_eligible_at` (same values as
...
blocked task. Each row carries server-derived `task_retry_available`,
`task_acknowledgement_available`, `next_action`, and `next_tool` fields. Follow
`next_tool`: only auxiliary cards that leave another visible recovery path use
`resolve_escalation`; eligible recovery-budget causes use `retry_blocked_task`,
dependency parks use `rearm_roadmap_item`, platform-policy conflicts use their
structured resolver, and unretryable stale work uses `replan_task`.
...
Acknowledges an escalation, recording `decision_md` as an audit note. Resolving
a project-pause escalation resumes the project. A blocked task's final visible
card and every platform-policy conflict are refused: use the typed transition
named by `list_escalations` so the work cannot disappear while staying blocked.
Idempotent.
...
After fixing the cause of an eligible `recovery_budget_exhausted_*` task block,
returns that one task to the queue with a fresh recovery episode and resolves
the block. Eligibility and tenant ownership are derived server-side. Other open
blockers prevent the retry. Idempotent on replay. This is a state-changing,
compute-spending action; confirm it with the operator. The response carries
`next_action: "done"`.
...
**Params:** `escalation_id: str`

Performs the same idempotent transition as the dashboard's **Re-arm this item**
action. Keelen verifies the delivered prerequisite from structural
`depends_on_item_ids` and exact linked PR identities, grants the same roadmap
item one expand retry, preserves its id and dependency list, and resolves its
park card in one transaction. It never treats a PR title or free-text citation
as structural lineage. A bounded live GitHub check may be used when local merge
state is stale.
...
**Params:** `escalation_id: str`, `decision: str`, `decision_md: str`, optional
`project_cap: int`

`decision` is one of `reuse_existing_evidence`, `split_task`,
`raise_project_cap`, or `remove_scenario`. The decision is written into a new
operator-approved plan section, the same task lineage is requeued, and only
...
**Params:** `project_id: str`, `task_id: str`, `title: str`, `body_md: str`,
`acceptance_criteria: list[str]`, `decision_md: str`

Replaces an unretryable blocked task under the same roadmap and attempt
lineage. The replacement receives pending acceptance criteria, inherited
prerequisites, and every downstream dependency; the stale task is cancelled
with `plan_superseded` provenance. Its exact PR attempts become terminal
`replanned` non-delivery records, so their SHA and failure history remain
auditable and no work is falsely marked delivered. Idempotent on replay.
...
or durably prioritizes and immediately attempts the next intake batch. Intake
is independent of the dev scheduler switch. `process_now` returns
`will_spawn_intake=true` only after a machine claim is persisted; otherwise it
returns `process_now_reason`, `process_now_raw_reason`, a truthful `next_step`,
the durable wake id, and request/override/election/machine/runner timestamps.
Inactive projects and credential, in-flight, daily-cap, quota, concurrency, or
runner-launch gates return false; an unconfirmed launch keeps the wake queued
and leaves `runner_started_at` empty until the hand-off is confirmed.
